...
LABOR_PENSION_LEVELS = [
    15000, 15840, 16500, 17280, 17880, 19047, 20008, 21009, 22000, 23100, 24000,
    25250, 26400, 27600, 28800, 30300, 31800, 33300, 34800, 36300, 38200, 40100,
    42000, 43900, 45800, 48200, 50600, 53000, 55400, 57800, 60800, 63800, 66800,
    69800, 72800, 76500, 80200, 83900, 87600, 92100, 96600, 101100, 105600, 110100,
    115500, 120900, 126300, 131100, 137100, 142500, 147900, 150000
]
...
def dual_input(label, key_prefix, min_value, max_value, default_val, step=1, format_fmt=None, help_text=None):
    st.markdown(f"**{label}**")
    ...
    with col1:
        st.number_input(
            label="數字輸入", min_value=min_value, max_value=max_value, step=step,
            format=format_fmt, key=num_key, on_change=sync_from_num, label_visibility="collapsed", help=help_text
        )
    with col2:
        st.slider(
            label="滑桿調整", min_value=min_value, max_value=max_value, step=step,
...
def estimate_labor_pension_balance(monthly_salary, self_rate, work_years, annual_return):
    ...
    estimated_balance = monthly_contrib * (((1 + r_month) ** total_months - 1) / r_month)
...
def run_initial_simulation(p):
    ...
    cur_bonus = safe_float(p.get("annual_bonus", 0))
    ...
    for age in range(current_age, end_age + 1):
        ...
        if age >= safe_int(p.get("labor_ins_claim_age", 60)):
            service_years = safe_float(p.get("labor_ins_years", 5)) + max(0, retire_age - current_age)
        ...
        eff_pension_ret = safe_float(p.get("labor_pension_return", 7.64)) if age < safe_int(p.get("labor_pension_claim_age", 60)) else safe_float(p.get("labor_pension_reinvest_return", 4.0))
        ...
        records.append({
            ...
            "備用金每月投入(含年金併入)": round(cash_m + labor_ins_annuity_monthly),
            "年度獎金/年終注入 (元)": round(annual_bonus_val),
...
def recalculate_dataframe(df, p):
    ...
    for _, row in df.iterrows():
        ...
        annual_bonus_val = safe_float(row.get("年度獎金/年終注入 (元)", 0))
        ...
        cur_cash = (cur_cash + cash_m * 12 + annual_bonus_val) * (1 + cash_ret / 100.0)
...
def Fast_simulate_end_asset(p, monthly_req_total, stock_ratio):
    ...
    for age in range(c_age, e_age + 1):
        ...
        if is_working:
            ...
            cash_in = (cash_m_working * 12) + bonus  # 💡 修正 1：獎金正確納入現金部位滾存
            ann_exp = safe_float(p.get("pre_retire_annual_expense", 120000))
        else:
            pension_contrib = 0.0
            stock_in = 0.0
            cash_in = 0.0
            ann_exp = safe_float(p.get("post_retire_expense", 60000)) * 12  # 💡 修正 2：退休後僅扣除退休月支出
...
def calculate_retirement_goal(p, stock_ratio=0.8):
    ...
    monthly_self_pension = level * self_rate
    ...
    if uploader_file is not None:
        try:
            ...
            for p_name, p in profiles.items():
                ...
                st.session_state[f"ez_c_age_{p_name}"] = safe_int(p.get("current_age", 30))
                st.session_state[f"ez_sal_{p_name}"] = safe_int(p.get("insured_salary", 50000))
                st.session_state[f"ez_r_age_{p_name}"] = safe_int(p.get("retire_age", 60))
                st.session_state[f"ez_w_yrs_{p_name}"] = safe_int(p.get("work_years", 5))
                st.session_state[f"ez_l_exp_{p_name}"] = safe_int(p.get("life_expectancy", 85))
                st.session_state[f"ez_pre_m_{p_name}"] = safe_int(p.get("pre_retire_monthly_expense", 30000))
                st.session_state[f"ez_pre_a_{p_name}"] = safe_int(p.get("pre_retire_annual_expense", 120000))
                st.session_state[f"ez_stk_b_{p_name}"] = safe_int(p.get("stock_balance", 1000000))
                st.session_state[f"ez_csh_b_{p_name}"] = safe_int(p.get("cash_balance", 500000))
                st.session_state[f"ez_post_m_{p_name}"] = safe_int(p.get("post_retire_expense", 60000))
                st.session_state[f"ez_leg_{p_name}"] = safe_int(p.get("legacy_target", 2000000))
                st.session_state[f"ez_sal_g_{p_name}"] = safe_float(p.get("salary_growth_rate", 1.0))
                st.session_state[f"ez_stk_r_{p_name}"] = safe_float(p.get("stock_return", 6.0))
                st.session_state[f"ez_lp_ret_pre_{p_name}"] = safe_float(p.get("labor_pension_return", 7.64))
                st.session_state[f"ez_csh_r_{p_name}"] = safe_float(p.get("cash_return", 2.0))
                st.session_state[f"ez_lp_ret_post_{p_name}"] = safe_float(p.get("labor_pension_reinvest_return", 4.0))
                st.session_state[f"ez_self_rate_{p_name}"] = safe_float(p.get("labor_pension_self_rate", 6.0))
                st.session_state[f"ez_custom_lp_{p_name}"] = safe_int(p.get("labor_pension_balance", 0))
